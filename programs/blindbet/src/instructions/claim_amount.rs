...
use crate::{ error::ErrorCode, state::{ Market, UserWager }, COMP_DEF_OFFSET_CLAIM_AMOUNT, ID };

#[init_computation_definition_accounts("claim_amount", payer)]
#[derive(Accounts)]
pub struct ClaimAmountCompDef<'info> {
    #[account(mut)]
...
#[queue_computation_accounts("claim_amount", signer)]
#[derive(Accounts)]
...
pub struct ClaimAmount<'info> {
    ...
    #[account(
        address = derive_comp_def_pda!(COMP_DEF_OFFSET_CLAIM_AMOUNT)
    )]
...
#[callback_accounts("claim_amount", signer)]
#[derive(Accounts)]
pub struct ClaimAmountCallback<'info> {
    #[account(mut)]
    pub signer: Signer<'info>,
